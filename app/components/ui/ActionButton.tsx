...
export function ActionButton({ icon: Icon, onClick, isDarkMode, highlight, active, danger, tooltip }: ActionButtonProps) {
    return (
        <button
            onClick={onClick}
            title={tooltip}
            className={`p-1.5 sm:p-2.5 rounded-lg sm:rounded-xl transition-all duration-200 relative group active:scale-95 ${highlight
        ...
        >
            <Icon size={18} strokeWidth={highlight ? 2.5 : 2} className="sm:hidden" />
            <Icon size={20} strokeWidth={highlight ? 2.5 : 2} className="hidden sm:block" />
        </button>
    );
}
