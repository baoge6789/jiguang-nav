...
export function WidgetDashboard({ isDarkMode, sitesCount, widgetStyle = 'B', widgetConfig }: WidgetDashboardProps) {
    ...
    <span className={`w-9 text-left shrink-0 ${isDarkMode ? 'opacity-60' : 'text-slate-600'}`}>{tz.name}</span>
    <span className={`w-6 text-right shrink-0 tabular-nums ${isDarkMode ? 'opacity-40' : 'text-slate-500'}`}>{dateStr}</span>
